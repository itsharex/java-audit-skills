...
**关键设计：**
1. **全局并发上限 = `{max_concurrent_agents}`（默认 5，启动时由用户确认；合法范围 2~10）**：任何阶段同时运行的 agent 总数（含 worker + 质检员）不得超过此值。负责人采用「波次（wave）调度」：每波至多挑 `{max_concurrent_agents}` 个就绪任务并行运行；同波内 worker 完成后立即让质检员就地校验，校验通过即关闭释放槽位，剩余任务进入下一波。**唯一例外**：负责人本身和侦查类 agent（agent-1-recon、agent-5-route-tracer）不计入上限（它们是单点协调者，不存在自身并发竞争）。
2. **质检员池按需扩缩**：负责人根据每个阶段的并发校验需求，动态 spawn agent-7-1, agent-7-2, ..., agent-7-N 质检员，确保每个完成的 agent 都能立即获得校验，零等待；但**质检员实例数 ≤ 当前阶段空闲并发槽位数**，整体仍受全局并发上限 `{max_concurrent_agents}` 约束
...
7. **阶段1 调度**：
   ...
   - **7.2 路由子流程**：
     ...
     - 负责人读取 `{output_path}/route_mapper/_recon_{recon_id}.md` 的「Agent 任务分配」表（recon_id 由 recon 生成且唯一），按表中 agent_id 与模块清单**按波次并行 spawn `agent-1-1`、`agent-1-2`、…、`agent-1-N`**：
       - 每波同时运行的 worker + 质检员总数 ≤ **`{max_concurrent_agents}`**（受全局并发上限约束）
       - 推荐配比：**`({max_concurrent_agents} - 1)` worker + 1 质检员**（worker 完成立即让质检员就地校验，校验通过释放槽位后再 spawn 下一个 worker）；多个 worker 同时完成时可降为 `({max_concurrent_agents} - 2)` worker + 2 质检员
       - 负责人本身不占槽位；agent-2/3 此时尚未启动（agent-2 依赖 merge），agent-3 与本子流程是"跨子流程并行"，应在主队列中与本子流程合并按 `{max_concurrent_agents}` 算（即 agent-3 占 1 槽，路由子流程剩 `{max_concurrent_agents} - 1` 槽）
   ...
   - **7.4 agent-2 启动**：`agent-1-merge` 校验通过后启动 agent-2-auth-audit；agent-2 完成后由空闲检员校验
   - **7.5 进入阶段2**：路由子流程 + agent-3 + agent-2 全部校验通过后，关闭本阶段质检员，并行启动 `agent-4a` 和 `agent-4b`
...
9. **阶段3 调度**：agent-5 分批完成后，负责人**按波次** spawn agent-5-1/5-2/.../5-N 并行追踪（每波 worker + 质检员 ≤ **`{max_concurrent_agents}`**，推荐 `({max_concurrent_agents} - 1)` worker + 1 质检员；agent-5-route-tracer 不计槽位）；每个 worker 完成后立即由空闲检员校验，通过后关闭该 worker 释放槽位，下一波继续 spawn；全部通过后负责人汇总覆盖率
10. **阶段4 调度**：负责人读取调用链报告，按 sink 类型按需启动 agent-6x；每个 agent-6x 完成后立即由空闲检员校验，通过后关闭（无对应 sink 则跳过，直接标记 completed）
11. **质检员池调度策略**：
...
4. **按分配规则生成 Agent 任务分配单**
   ...
   **执行顺序（强制，禁止跳步）：**
   1. 凡**第 1 层物理模块预估路由 > 50**，必须先完成第 2 层逻辑模块下钻（按上文「逐模块按框架切分逻辑模块」表展开为 namespace / 包前缀 / endpoint 级条目）；**未下钻的物理模块禁止直接进入本步分配规则**。
   2. 分配规则的判定单元是**「逻辑模块」**（namespace / 包前缀 / endpoint / 单文件），不是物理模块（WAR / 子模块）。
   3. 物理模块整体 ≤ 50 路由 且 无通配符，方可走「整 WAR 给 1 agent」的捷径（不下钻）。
   ...
   **分配规则（强制）：**
   ...
   - 含通配符 / `executeInterface` / Struts2 `*_*` 双通配 → **强制独占**（无视路由数）
     ...
     - 正例（要求）：该 WAR → 第 2 层展开 N 个 namespace 逻辑模块 → 含通配符的 namespace 各自独占、其余按合并/独占规则 → 产出 agent-1-1 ~ agent-1-N。
     - 该规则的真实目的：**通配符路由数无法静态精确预估**（用 `class_count × 8` 上界），让含通配符的逻辑模块独占可避免单 agent 超载；并非"避免跨 agent 协调通配符"。
   ...
   - **整 WAR 单 agent 承包的两个前置条件**（必须同时满足，缺一不可）：(a) 物理模块预估路由 ≤ 50；(b) 物理模块**完全不含**通配符 / `executeInterface` / 反射分发。任一不满足即禁止整 WAR 单 agent，必须下钻到第 2 层后按本规则重新分配
...
- **按需创建**：某个 agent 完成任务后，负责人立即 spawn 一个质检员校验其输出；不提前批量预创建
  ...
  - 阶段3：每个 agent-5-N worker 完成后，按需 spawn 质检员（受全局并发上限 `{max_concurrent_agents}` 约束，质检员实例数 ≤ `{max_concurrent_agents}` - 当前活跃 worker 数）
