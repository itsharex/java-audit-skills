...
4. **按分配规则生成 Agent 任务分配单**
   ...
   **分配规则（强制）：**
   ...
   - 含通配符 / `executeInterface` / Struts2 `*_*` 双通配 → **强制独占**（无视路由数）
     ...
     - 反例（禁止）：admin WAR（1680 路由 / 27 namespace，部分含 `*_*`）→ 整个 admin 给 agent-1-1 独占。
     - 正例（要求）：admin WAR → 第 2 层展开 27 个 namespace 逻辑模块 → 含通配符的 namespace 各自独占、其余按合并/独占规则 → 产出 agent-1-1 ~ agent-1-N。
