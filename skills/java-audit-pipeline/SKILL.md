...
5. **阶段1 调度**：
   ...
   - **5.2 路由子流程**：
     ...
     - 负责人读取 `{output_path}/route_mapper/_recon_{recon_id}.md` 的「Agent 任务分配」表（recon_id 由 recon 生成且唯一），按表中 agent_id 与模块清单**并行 spawn `agent-1-1`、`agent-1-2`、…、`agent-1-N`**（每个 worker 调用 `/java-route-mapper` 但只处理自己负责的模块路径列表）
